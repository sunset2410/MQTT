...
map<int, CallBackPointers> mapCallbacks;
std::mutex mMosqCallbackMutex;
...
{
    auto it = mapCallbacks.find(dcepf_handle_t);
    if (it == mapCallbacks.end())
    {
        cout << "Element not found";
        return mapCallbacks.end();
    }
    else
    {
        return it;
...
void on_connect_v5(struct mosquitto *, void *obj, int rc, int flag, const mosquitto_property *props)
{
    ...
    it->second.dcepf_mqtt_sp_notify_connack_cb_t(dcepf_handle_t, obj, rc, flag, props);
}
...
void on_subscribe_v5(struct mosquitto *mosq, void *obj, int mid, int qos_count, const int *granted_qo, const mosquitto_property *props)
{
    ...
    int dcepf_handle_t = client->get_dcepf_handle_t();
    auto it = getCallBackPointer(dcepf_handle_t);
    it->second.dcepf_mqtt_sp_notify_suback_cb_t(dcepf_handle_t, obj, mid, qos_count, granted_qo, props);
}
...
bool dcepf_mqtt_sp_invoke_client_init(int dcepf_handle_t, const char *id, void callback(int handle, dcepf_result_t result, mosquitto *instance))
{
    ...
    CallBackPointers callBackPointer;
    callBackPointer.dcepf_mqtt_sp_invoke_client_init_cb_t = callback;

    mapCallbacks.insert(pair<int, CallBackPointers>(dcepf_handle_t, callBackPointer));
    ...
    if (it == mapCallbacks.end())
    {
        it->second.dcepf_mqtt_sp_invoke_client_init_cb_t(dcepf_handle_t, DCEPF_RESULT_FAILED, instance);
    }
    else
    {
        it->second.dcepf_mqtt_sp_invoke_client_init_cb_t(dcepf_handle_t, DCEPF_RESULT_SUCCESS, instance);
    }
}
...
{
    // cout << "dcepf_mqtt_sp_start_connack_notify" << endl;
    auto it = getCallBackPointer(dcepf_handle_t);
    it->second.dcepf_mqtt_sp_notify_suback_cb_t = callback;
    return getMqttClient(dcepf_handle_t)->dcepf_mqtt_sp_start_suback_notify(on_subscribe_v5);
}
...
bool dcepf_mqtt_sp_start_connack_notify(int dcepf_handle_t, void callback(int dcepf_handle_t, void *obj, int rc, int flag, const mosquitto_property *props))
{
    // cout << "dcepf_mqtt_sp_start_connack_notify" << endl;
    auto it = getCallBackPointer(dcepf_handle_t);
    it->second.dcepf_mqtt_sp_notify_connack_cb_t = callback;
    return getMqttClient(dcepf_handle_t)->dcepf_mqtt_sp_start_connack_notify(on_connect_v5);
}
...
{
    // cout << "dcepf_mqtt_sp_start_message_notify" << endl;
    auto it = getCallBackPointer(dcepf_handle_t);
    it->second.dcepf_mqtt_sp_notify_message_cb_t = callback;
    return getMqttClient(dcepf_handle_t)->dcepf_mqtt_sp_start_message_notify(on_message_v5);
}
...
{
    // cout << "dcepf_mqtt_sp_start_disconnect_notify" << endl;
    auto it = getCallBackPointer(dcepf_handle_t);
    it->second.dcepf_mqtt_sp_notify_disconnect_cb_t = callback;
    return getMqttClient(dcepf_handle_t)->dcepf_mqtt_sp_start_disconnect_notify(on_disconnect_v5);
}
...
{
    // cout << "dcepf_mqtt_sp_start_puback_notify" << endl;
    auto it = getCallBackPointer(dcepf_handle_t);
    it->second.dcepf_mqtt_sp_notify_puback_cb_t = callback;
    return getMqttClient(dcepf_handle_t)->dcepf_mqtt_sp_start_puback_notify(on_publish_v5);
}
bool dcepf_mqtt_sp_start_unsuback_notify(int dcepf_handle_t, void callback(int dcepf_handle_t, void *, int, const mosquitto_property *props))
{
    // cout << "dcepf_mqtt_sp_start_puback_notify" << endl;
    auto it = getCallBackPointer(dcepf_handle_t);
    it->second.dcepf_mqtt_sp_notify_unsuback_cb_t = callback;
    return getMqttClient(dcepf_handle_t)->dcepf_mqtt_sp_start_unsuback_notify(on_unsubscribe_v5);
}
